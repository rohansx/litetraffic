...
| `scenario.lock.json` | `{"manifest", "engine", "files"}`: the parsed manifest, the k6 version string, and the sha256 of the script, of every file in its relative import closure, and of the `fixtures.pool` file when set, keyed by path inside the bundle |
...
When any journey declares `expected_statuses`, `metrics.unexpected_http_failure_rate` holds `samples` (all `http_req_failed` points), `failed`, and `rate`. `failed` counts `http_req_failed` points with value 1 whose k6 `status` tag is not listed for the point's `operation` tag; operations without a declaration keep every k6 failure, and transport failures (status `"0"`) always count. `metrics.http_req_failed_rate` is unchanged. The key is absent when no journey declares `expected_statuses` or k6 recorded no failure points. When it is present, `report.html` adds an Unexpected HTTP failure rate row, the `verify` text summary adds `unexpected HTTP failure rate: X% (FAILED/SAMPLES)`, and the dashboard run page lists it with the other metrics. The rate is descriptive and does not affect the verdict.
...
`performance.by_operation` reports `baseline_p95_ms`, `candidate_p95_ms`, and `change_percent` for each operation present with a p95 in both runs (`change_percent` is `null` when the baseline p95 is not positive). Per-operation changes are descriptive and do not affect the verdict or the p95 gate, which uses the overall p95. When either run has `metrics.unexpected_http_failure_rate`, `performance.unexpected_http_error_rate` holds `baseline`, `candidate` (`null` when that run lacks it), and `change_percentage_points`, shaped like `performance.http_error_rate`; it is descriptive and does not affect the verdict.
