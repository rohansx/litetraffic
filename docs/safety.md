...
| Budget | Enforced how |
|---|---|
| `max_seconds` | Before the run, the schedule plus fixture/observer deadlines must fit. At run time, k6's whole process group is stopped (SIGTERM, then SIGKILL) when its share expires, and a group that is still alive afterwards adds the limitation `k6 process group did not exit`; the run becomes `timed_out` |
