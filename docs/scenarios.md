...
| `endpoints` | yes | `[{"method", "path", "kind": "read"\|"write", "name"?, "body"?, "attack_body"?, "expected_statuses"?, "check_own"?, "read_back"?, "journey_in_path"?}]`. Placeholders are filled from the resource under test: URL-encoded in `path`, verbatim in every `body` string (a `{name}` that is not a resource key stays literal). `{journey}` is filled with the journey key (`lt.journeyKey()`, `<run_id>-<scenario>-<iteration>`) in every body string, and in the path only when the endpoint sets `journey_in_path: true`, so rows a write creates can be tied back to the journey (and run) that made them; `journey` cannot be a resource key. `path` may otherwise only use resource keys, and each endpoint must use at least one placeholder in its path or body, so `POST /positions` with `{"organization_id": "{id}"}` works (given an `attack_body`, below, since that body has no string to vary). Reads use `GET`/`HEAD`, writes `POST`/`PUT`/`PATCH`/`DELETE` with an optional JSON `body`. `expected_statuses` are the owner's success statuses (default `[200]` for reads, `[200, 201, 204]` for writes). At least one read is required. A write reads its resource back through its `read_back`: the index of a `GET` read endpoint in `endpoints` or that read's `name` (default: the first `GET` read); a reference to anything but a read endpoint, or to a `HEAD` read (no body to compare), is rejected. `attack_body` is what the other identity writes (placeholders filled as in `body`); when absent it is `body` with every string value suffixed by `-lt-attack-{journey}`, at any depth, except strings that use a resource placeholder (they address the victim's resource, so changing them would aim the attack elsewhere). The owner (`check_own`) writes `body` and the attacker writes `attack_body`, so the two payloads differ, though a field the server stores may still be equal in both (`{"qty": 5, "note": "n"}` varies only `note`); when the read-back already holds the attacker's value for every field it shares with it, `victim_unchanged` is `unknown` (below), so give such bodies an `attack_body` that changes a stored field. When the attacker body would equal `body` (no string to vary, as with numbers only or only resource placeholders, or no body on a `check_own` write), or an explicit `attack_body` equals `body`, `init` fails asking for an `attack_body`. A write with no `body` and no `check_own` (a plain `DELETE /notes/{id}`) needs none: the attacker sends the same bodyless request, and there is no owner write to pre-match it. `read_back` is only checked on writes, so a config whose reads are all `HEAD` is valid as long as it has no writes |
