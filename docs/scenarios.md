...
| `endpoints` | yes | `[{"method", "path", "kind": "read"\|"write", "body"?, "expected_statuses"?, "check_own"?}]`. Placeholders are filled from the resource under test: URL-encoded in `path`, verbatim in every `body` string (a `{name}` that is not a resource key stays literal). `path` may only use resource keys, and each endpoint must use at least one placeholder in its path or body, so `POST /positions` with `{"organization_id": "{id}"}` works. Reads use `GET`/`HEAD`, writes `POST`/`PUT`/`PATCH`/`DELETE` with an optional JSON `body`. `expected_statuses` are the owner's success statuses (default `[200]` for reads, `[200, 201, 204]` for writes). At least one read is required; the first read is the read-back |
