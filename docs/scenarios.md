...
| `identities` | yes | Exactly two, each `{"name", "resources", "markers"?, "auth"?, "token_env"?, "headers"?}`. `resources` are what this identity owns: each is an id (the `{id}` placeholder) or an object of named placeholders such as `{"id": "org-a", "position": "pos-a"}`; every resource must have the same keys. Give `auth` (an [actor auth](#actor-auth) recipe; `kind` defaults to `jwt_hs256`) or `token_env` (a variable holding a ready bearer token), not both; `headers` are plain, non-secret headers. At least one of the three is required. `markers` are non-empty strings that only this identity's private data contains, such as a seeded secret value or row id (resource ids are not used automatically); see the body checks below. A marker equal to or contained in one of the other identity's markers is rejected, since that identity's own data would then read as a leak. Markers are written into `journeys.js` as plain text, so use seeded test values, not real secrets |
| `endpoints` | yes | `[{"method", "path", "kind": "read"\|"write", "name"?, "body"?, "attack_body"?, "expected_statuses"?, "check_own"?, "read_back"?, "journey_in_path"?}]`. Placeholders are filled from the resource under test: URL-encoded in `path`, verbatim in every `body` string (a `{name}` that is not a resource key stays literal). `{journey}` is filled with the journey key (`lt.journeyKey()`, `<run_id>-<scenario>-<iteration>`) in every body string, and in the path only when the endpoint sets `journey_in_path: true`, so rows a write creates can be tied back to the journey (and run) that made them; `journey` cannot be a resource key. `path` may otherwise only use resource keys, and each endpoint must use at least one placeholder in its path or body, so `POST /positions` with `{"organization_id": "{id}"}` works (given an `attack_body`, below, since that body has no value to vary). Reads use `GET`/`HEAD`, writes `POST`/`PUT`/`PATCH`/`DELETE` with an optional JSON `body`. `expected_statuses` are the owner's success statuses (default `[200]` for reads, `[200, 201, 204]` for writes). At least one read is required. A write reads its resource back through its `read_back`: the index of a `GET` read endpoint in `endpoints` or that read's `name` (default: the first `GET` read); a reference to anything but a read endpoint, or to a `HEAD` read (no body to compare), is rejected. `attack_body` is what the other identity writes (placeholders filled as in `body`); when absent it is `body` with every value changed, at any depth: strings suffixed by `-lt-attack-{journey}`, numbers incremented by 1, booleans negated and `null` fields replaced by `"lt-attack-{journey}"`, except strings that use a resource placeholder (they address the victim's resource, so changing them would aim the attack elsewhere; such fields are equal in both bodies). The owner (`check_own`) writes `body` and the attacker writes `attack_body`, so every generated field differs from what the owner wrote (`{"qty": 5, "note": "n"}` becomes `{"qty": 6, "note": "n-lt-attack-..."}`); an explicit `attack_body` is used as written and may share values with `body`. Whatever the body, when the read-back cannot show the attacker's write (below), `victim_unchanged` is `unknown`, never a pass. When the attacker body would equal `body` (nothing to vary, as with only resource placeholders, `{}`, or no body on a `check_own` write), or an explicit `attack_body` equals `body`, `init` fails asking for an `attack_body`. A write with no `body` and no `check_own` (a plain `DELETE /notes/{id}`) needs none: the attacker sends the same bodyless request, and there is no owner write to pre-match it. `read_back` is only checked on writes, so a config whose reads are all `HEAD` is valid as long as it has no writes |
...
- `victim_unchanged` (only with write endpoints): the owner reads the resource back (through the write's `read_back`) before and after each cross-tenant write; the statuses and bodies must match (`deepEqual`). This catches a server that answers `403` but applies the write anyway. When the read-back before the attack cannot show a mutation (an empty body, `null`, `{}` or `[]`, or a state that already holds the attacker body: every field of an object attacker body that the state also has already has that value, or the state has none of its fields, since fields the server ignores cannot show the write either; or an equal non-object body), the journey reports no `victim_unchanged` event unless another write in it failed, so the assertion is `unknown` (partial evidence) and the verdict `inconclusive` rather than a pass it cannot back.
...
Limits: owner writes are off by default because a concurrent journey's owner write would race another journey's read-back compare; set `check_own` only for idempotent writes. Each `token_env` name is listed in the generated manifest's `secret_env`, so its value is scrubbed from kept artifacts like any declared credential (values shorter than 8 characters are not). Prefer `auth` when the app accepts controller-minted JWTs. Resource ids are static; regenerate after editing the config rather than editing the generated files.
...
- Tokens and signing keys are never written by the controller. Every declared credential value — actor `auth` signing keys, minted `LT_TOKEN_*` JWTs and each token in `LT_TOKENS_*`, fixture and observer `bearer_token_env` tokens, observation `headers_env` values and the variables named in the manifest's `secret_env` — is replaced with `[redacted]` in everything a run keeps or prints: `engine.stdout.log`, `engine.stderr.log`, `console.log` and `metrics.jsonl` are scrubbed as text for the literal value and its JSON-escaped forms (Python's `json.dumps` spellings and Go's `encoding/json` HTML escapes), so script events are scrubbed before assertions read them — their pass flags come from the script. Every file in the run directory is scrubbed this way on every exit path — finished, timed out, crashed, cancelled by a Ctrl-C at any point (including mid-redaction), or an unexpected error — before final artifacts are written; a file that cannot be scrubbed is deleted and the run records the limitation `NAME could not be scrubbed of secrets and was deleted`. `events/`, `observation.json`, `fixture.json`, `result.json` (and so `report.html`, the dashboard and `verify --json`) are also scrubbed by walking every decoded JSON string, keys included, which catches any other escape spelling; observations and fixtures are evaluated against the real values first. Values shorter than 8 characters are not scrubbed. `inspect` lists the `secret_env` names.
