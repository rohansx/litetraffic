...
`setup` and `teardown` are argv lists, run directly without a shell, so write `["sh", "-c", "..."]` yourself if you need one. Both run in the scenario directory (`cwd` accepts only `"bundle"`), with the caller's environment plus `LT_RUN_ID`, `LT_TARGET` and `LT_SEED`; put connection settings such as `PGHOST` or `DATABASE_URL` in the environment, not in the manifest. `timeout_seconds` (1–60) applies to each command, and both timeouts, each plus a 4-second SIGTERM/SIGKILL grace, are reserved from `max_seconds` before k6 gets its share.
